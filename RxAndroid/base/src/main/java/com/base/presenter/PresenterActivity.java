...
import com.base.bus.RxSubscriptions;
import com.base.model.BindModel;
 ...
 * Created by Travis1022 on 17/02/24.
 */
public abstract class PresenterActivity<VB extends View> extends RxAppCompatActivity implements Presenter {
    ...
    private <M extends Model> void getModelProcessor() {
        ...
        try {
            ...
            for (Field field : fields) {
                ...
                if (annotation != null) {
                    if (field.getModifiers() != Modifier.PUBLIC) AndroidLogger.e("ModelProcessor不能使用修饰符");
    ...
    protected final <T> Flowable<T> receiver(String tag) {
        Flowable<T> observable = (Flowable<T>) mObservableMap.remove(tag + View.TAG);
        if (observable != null)
    ...
    protected final <T> Flowable<T> receiverFlowable(String tag) {
        return (Flowable<T>) mObservableMap.remove(tag + View.TAG);
    }
}
