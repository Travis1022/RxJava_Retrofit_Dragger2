...
import com.sunnybear.library.widget.zoom.decoder.CompatDecoderFactory;
import com.sunnybear.library.widget.zoom.decoder.DecoderFactory;
import com.sunnybear.library.widget.zoom.decoder.ImageDecoder;
import com.sunnybear.library.widget.zoom.decoder.ImageRegionDecoder;
import com.sunnybear.library.widget.zoom.decoder.SkiaImageDecoder;
import com.sunnybear.library.widget.zoom.decoder.SkiaImageRegionDecoder;
...
public class ZoomImageView extends View {
    ...
    public ZoomImageView(Context context, AttributeSet attr) {
        ...
        this.handler = new Handler(new Handler.Callback() {
            public boolean handleMessage(Message message) {
                if (message.what == MESSAGE_LONG_CLICK && onLongClickListener != null) {
                    maxTouchCount = 0;
                    ZoomImageView.super.setOnLongClickListener(onLongClickListener);
                    performLongClick();
        ...
        if (attr != null) {
            TypedArray typedAttr = getContext().obtainStyledAttributes(attr, R.styleable.ZoomImageView);
            if (typedAttr.hasValue(R.styleable.ZoomImageView_assetName)) {
                String assetName = typedAttr.getString(R.styleable.ZoomImageView_assetName);
                if (assetName != null && assetName.length() > 0) {
                    setImage(ImageSource.asset(assetName).tilingEnabled());
                }
            }
            if (typedAttr.hasValue(R.styleable.ZoomImageView_src)) {
                int resId = typedAttr.getResourceId(R.styleable.ZoomImageView_src, 0);
                if (resId > 0) {
                    setImage(ImageSource.resource(resId).tilingEnabled());
                }
            }
            if (typedAttr.hasValue(R.styleable.ZoomImageView_panEnabled)) {
                setPanEnabled(typedAttr.getBoolean(R.styleable.ZoomImageView_panEnabled, true));
            }
            if (typedAttr.hasValue(R.styleable.ZoomImageView_zoomEnabled)) {
                setZoomEnabled(typedAttr.getBoolean(R.styleable.ZoomImageView_zoomEnabled, true));
            }
            if (typedAttr.hasValue(R.styleable.ZoomImageView_quickScaleEnabled)) {
                setQuickScaleEnabled(typedAttr.getBoolean(R.styleable.ZoomImageView_quickScaleEnabled, true));
            }
            if (typedAttr.hasValue(R.styleable.ZoomImageView_tileBackgroundColor)) {
                setTileBackgroundColor(typedAttr.getColor(R.styleable.ZoomImageView_tileBackgroundColor, Color.argb(0, 0, 0, 0)));
            }
            typedAttr.recycle();
        }
    ...
    private void doubleTapZoom(PointF sCenter, PointF vFocus) {
        ...
        float doubleTapZoomScale = Math.min(maxScale, ZoomImageView.this.doubleTapZoomScale);
    ...
    private static class TilesInitTask extends AsyncTask<Void, Void, int[]> {
        ...
        public TilesInitTask(ZoomImageView view, Context context, DecoderFactory<? extends ImageRegionDecoder> decoderFactory, Uri source) {
        ...
        protected int[] doInBackground(Void... params) {
            try {
                ...
                ZoomImageView view = viewRef.get();
    ...
    private static class TileLoadTask extends AsyncTask<Void, Void, Bitmap> {
        ...
        public TileLoadTask(ZoomImageView view, ImageRegionDecoder decoder, Tile tile) {
            this.viewRef = new WeakReference<>(view);
    ...
    private static class BitmapLoadTask extends AsyncTask<Void, Void, Integer> {
        ...
        public BitmapLoadTask(ZoomImageView view, Context context, DecoderFactory<? extends ImageDecoder> decoderFactory, Uri source, boolean preview) {
            this.viewRef = new WeakReference<>(view);
            this.contextRef = new WeakReference<>(context);
        ...
        protected Integer doInBackground(Void... params) {
            try {
                ...
                ZoomImageView subsamplingScaleImageView = viewRef.get();
