...
public abstract class LoadMoreAdapter<Item extends Serializable, VH extends QuickViewHolder> extends QuickAdapter<Item, VH> {
     ...
     * @return viewType
     */
    public int getMultiItemViewType(Item item, int position) {
        return 0;
    }
    ...
    @Override
    public final int getItemViewType(Item item, int position) {
        if (position == getItemCount() - 1)//最后一个位置
            viewType = TYPE_FOOTER;
        else
            viewType = getMultiItemViewType(item, position);
        return viewType;
    }
